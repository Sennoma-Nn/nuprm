...
def create-left-prompt [] {
    ...
    let host_name = get-prompt-info host-name -l $"($status_color) @ ($colors.white)"
    let user_name = $"($colors.white)(get-prompt-info user-name)"
    let user_host = $"($user_name)($host_name)"
    let path_info = get-prompt-info path -u (if (get-prompt-info path-mode) == "DOS" { "\\" } else { "/" }) -d $"($status_color)" -s $"($colors.white)" -r "\e[0m" -u
    let path_show = $"[ ($path_info) ($colors.white)]"
    let git_info = (get-prompt-info git -l $" in ($status_color)" -r $colors.reset)
    let execution_time = if (get-prompt-info exec-time | into float) > 0.5 { $" ($status_color)- ($colors.white)(get-prompt-info exec-time)sec" } else { "" }
